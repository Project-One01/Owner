const APPS_SCRIPT_URL = `https://script.google.com/macros/s/AKfycbxTYRzE9HzagQm0KhkFulXoef8jCBLBobsek2MVB9a-8cUUdHl1SKgh2X6I5SrRhtzv/exec`;
...
function getLocalDateString(dateInput) {
  ...
  if (isNaN(date.getTime())) {
    console.error("[getLocalDateString] Invalid date:", dateInput);
    return getTodayWIB();
  }
  const wibTime = new Date(
...
function updateDateTime() {
  ...
  const options = {
    ...
    hour: "2-digit",
    minute: "2-digit",
    timeZone: "Asia/Jakarta", // ← CRITICAL: Paksa WIB
  };
...
function getDisplayLabel(item, barangData) {
  // Cari data barang asli untuk mendapatkan jenisKelompok
  const barang = barangData ? barangData.find((b) => b.id === item.id) : null;
  const jenisKelompok =
    item.jenisKelompok || (barang ? barang.jenisKelompok : null) || "Satuan";
...
function calculateTotalStokForBarang(
  idBarang,
  barangDataArray,
  source = "BM1"
) {
...
function getBarangModalPrice(barang, isKelompok) {
  ...
  if (isKelompok) {
    ...
    return banyakItemPerTurunan > 0
      ? modalBarang / banyakItemPerTurunan
      : modalBarang;
  }
}
...
async function searchCustomerDeposit(searchQuery) {
  const response = await sendToSheet("SEARCH_CUSTOMER_DEPOSIT", {
    searchQuery,
  });
  if (!response.success) return [];
...
async function loadBarangData() {
  ...
  if (response.success) {
    return response.result.map((barang) => {
      ...
      const totalStokBM1 =
        stokKelompokBM1 * banyakItemPerTurunan + stokTurunanBM1;
      const totalStokBM2 =
        stokKelompokBM2 * banyakItemPerTurunan + stokTurunanBM2;
      return {
...
async function saveBarangData(data) {
  ...
  const validatedData = dataToSave.map((barang) => {
    const jenisKelompok = barang.jenisKelompok || "satuan";
    const banyakItemPerTurunan =
      barang.banyakItemPerTurunan || getBanyakItemPerTurunan(jenisKelompok);
    ...
    console.log("[SAVE_BARANG] Sending:", {
      id: barang.id,
      nama: barang.nama,
      jenisKelompok,
      banyakItemPerTurunan,
      stokKelompokBM1,
      stokTurunanBM1,
      stokKelompokBM2,
      stokTurunanBM2,
      modalBarang: barang.modalBarang,
      hargaJualKelompok: barang.hargaJualKelompok,
      hargaJualTurunan: barang.hargaJualTurunan,
    });
    return {
...
async function saveTransaksiData(transaksi, detailData = []) {
  ...
  const validatedTransaksi = transaksiArray.map((trans) => {
    console.log("[SAVE_TRANSAKSI_API] Validating transaction:", {
      id: trans.id,
      total: trans.total,
      bayar: trans.bayar,
      sisa: trans.sisa,
    ...
    return {
      ...
      bayar: parseFloat(trans.bayar) || 0, // ✅ PASTIKAN DI-PARSE
      sisa: parseFloat(trans.sisa) || 0, // ✅ PASTIKAN DI-PARSE
  ...
  const validatedDetailData = detailData.map((detail) => {
    console.log("[SAVE_TRANSAKSI_API] Detail item:", {
      id: detail.id,
      nama: detail.nama,
      diskonPersen: detail.diskonPersen,
      diskonRupiah: detail.diskonRupiah,
    });
    ...
    return {
      ...
      qty: detail.qty || 0,
      typeBarang:
        detail.typeBarang || (detail.isKotak ? "kelompok" : "turunan"),
  ...
  console.log("[SAVE_TRANSAKSI_API] Sending to server:", {
    transactions: validatedTransaksi.length,
    details: validatedDetailData.length,
    sampleTrans: validatedTransaksi[0],
  });

  return await sendToSheet("SAVE_TRANSAKSI", {
    data: validatedTransaksi, // ✅ Gunakan data yang sudah divalidasi
...
async function deleteBarangById(id) {
  return await sendToSheet("DELETE_BARANG", { id });
}
...
function terbilang(angka) {
  ...
  // ✅ PERBAIKAN: Handle 0 dengan benar
...
// ✅ TEST FUNCTION TERBILANG
console.log("[TEST TERBILANG]");
console.log("80000 =", terbilang(80000)); // Harusnya: "Delapan Puluh Ribu"
console.log("20000 =", terbilang(20000)); // Harusnya: "Dua Puluh Ribu"
console.log("50 =", terbilang(50)); // Harusnya: "Lima Puluh"
console.log("100 =", terbilang(100)); // Harusnya: "Seratus"

function initGlobalAPI() {
  console.log("[GLOBAL_API] Initializing...");
  updateDateTime();
  setInterval(updateDateTime, 1000);
  updateConnectionStatus();
  setInterval(updateConnectionStatus, 5000);
  console.log("[GLOBAL_API] Ready!");
}
